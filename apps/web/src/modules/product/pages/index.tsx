...
import { Product, useProductsQuery } from '@/services/graphql';
    ...
    <>
      <div className="mb-5 sm:flex sm:items-center sm:justify-between">
        ...
        <div className="mb-4 sm:mb-0">
          <h1 className="text-2xl font-bold text-slate-800 md:text-3xl">Product ✨</h1>
        </div>
      ...
      <Table
        pagination={{
          total: data?.products.totalDocs || 0,
          showSizeChanger: true,
          showQuickJumper: true,
          current: data?.products.page || 1,
        ...
        className="w-full"
        dataSource={data?.products.docs as Product[]}
        columns={columns}
      />

      <ProductEditDrawer open={open} onClose={() => setOpen(false)} />
      <ProductCreateModal open={showCreateModal} onCancel={() => setShowCreateModal(false)} />
    </>
  );
};
